...
Thanks for your interest — CONDUCTOR is built in the open and welcomes contributions of every size, from typo fixes to whole new LLM backends.
...
The [open issues](https://github.com/D-sorganization/CONDUCTOR/issues) are the authoritative list. Good first issues are labelled `good-first-issue`. If you want to tackle something bigger (a new backend, the GUI, the gRPC API), comment on the tracking issue first so we can align on design.

## How to add a new LLM backend

1. Create `conductor/backends/<name>.py` implementing `ILLMBackend`.
2. Register it at the bottom: `registry.register("<name>", YourBackend)`.
3. Import it in `conductor/backends/registry.py::_autoload()`.
...
Use the bug-report issue template. Include: CONDUCTOR version, Python version, OS, config (redact secrets), and a minimal reproduction.
