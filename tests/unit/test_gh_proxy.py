...
class FakeGitHubClient:
    ...
    async def set_issue_state(self, **kwargs: Any) -> str:
        self.calls.append(("set_issue_state", kwargs))
        return "https://github.com/test/issues/1"
...
class TestMakeGhProxy:
    ...
    @pytest.mark.asyncio
    async def test_set_state_dispatches_to_issue_state_client(
        self, fake_client: FakeGitHubClient
    ) -> None:
        tool = make_gh_proxy(
            client=fake_client,  # type: ignore[arg-type]
            allowlist=GhProxyAllowlist(frozenset({"set_state"})),
            task_id="t-123",
        )
        result = await tool(
            "set_state",
