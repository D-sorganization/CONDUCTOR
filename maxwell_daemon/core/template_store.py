    ...
    def _load_builtins(self) -> None:
        ...
        builtins = [
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Scan the repository {{ repo }} for TODOs and FIXMEs. "
                    "Group them by file and suggest a priority order for addressing them."
                ),
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Analyze {{ target_dir }} for duplicated code blocks. "
                    "Provide a report of the top 5 largest duplicated sections "
                    "and suggest refactoring strategies to apply DRY principles."
                ),
                parameters=[
                    TemplateParameter(
                        name="target_dir",
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Scan the Python files in {{ target_dir }} for dead code "
                    "(unused imports, uncalled functions, inaccessible classes). "
                    "Produce a summary of what can be safely deleted."
                ),
                parameters=[
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Investigate bug report #{{ issue_number }}. "
                    "Review the traceback and the code in {{ repo }}. "
                    "Identify the root cause and propose a step-by-step fix."
                ),
                parameters=[
                    TemplateParameter(
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Write a failing test case in {{ test_file }} that reproduces "
                    "the behavior described in bug report #{{ issue_number }}."
                ),
                parameters=[
                    TemplateParameter(
                        name="issue_number", type=ParameterType.STRING, required=True
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Write release notes for {{ repo }} comparing the current state "
                    "against {{ previous_tag }}. Focus on user-facing features, "
                    "bug fixes, and breaking changes. Format in markdown."
                ),
                parameters=[
                    TemplateParameter(name="repo", type=ParameterType.REPO, required=True),
                    TemplateParameter(
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Read the git log of {{ repo }} since {{ previous_tag }} "
                    "and append a new section to CHANGELOG.md following the "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review {{ file_path }}. Add PEP 257 compliant docstrings "
                    "to all functions and classes that are missing them. "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review the codebase in {{ repo }} and update the README.md "
                    "to ensure the feature list, setup instructions, and usage "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review the changes in PR #{{ pr_number }} for {{ repo }}. "
                    "Check for logic errors, security issues, performance "
                    "bottlenecks, and style violations. Produce a consolidated review."
                ),
                parameters=[
                    TemplateParameter(name="repo", type=ParameterType.REPO, required=True),
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Read issue #{{ issue_number }} in {{ repo }}. "
                    "Create a new branch, implement the requested fix or feature, "
                    "ensure tests pass, and create a Pull Request resolving the issue."
                ),
                parameters=[
                    TemplateParameter(name="repo", type=ParameterType.REPO, required=True),
                    TemplateParameter(
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Write comprehensive pytest unit tests for the functions in "
                    "{{ file_path }}. Place the tests in {{ test_path }} "
                    "and aim for high coverage of edge cases."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True),
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Run the test suite in {{ target_dir }}. "
                    "Identify any failing tests, diagnose the root cause "
                    "(whether the code is broken or the test is outdated), "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Refactor {{ file_path }} to use Python's async/await "
                    "where appropriate (e.g., I/O bound operations). "
                    "Ensure backwards compatibility or update callers accordingly."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Add strict Python type hints to all function signatures "
                    "and complex variables in {{ file_path }}. "
                    "Run mypy to ensure there are no typing errors."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Perform a security audit on {{ target_dir }}. "
                    "Look for SQL injection, hardcoded secrets, XSS vulnerabilities, "
                    "and unsafe file I/O. Provide a remediation report."
                ),
                parameters=[
                    TemplateParameter(name="target_dir", type=ParameterType.STRING, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Check the dependencies defined in {{ file_path }} "
                    "for newer secure versions. Update the file and ensure "
                    "that `pip install` works without resolution conflicts."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Analyze {{ file_path }} for computational or I/O bottlenecks. "
                    "Suggest and implement optimizations to reduce time complexity "
                    "or unnecessary memory allocations."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True)
                ],
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Review the SQL queries or ORM calls in {{ file_path }}. "
                    "Suggest indexes, N+1 query fixes, or query rewrites "
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Create a new Typer CLI command in {{ file_path }} "
                    "named `{{ command_name }}`. It should accept options "
                    "for `verbose` and `dry-run`, and include basic docstrings."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True),
                    TemplateParameter(
            ...
            TaskTemplate(
                ...
                prompt_template=(
                    "Add a new `{{ method }}` route at `{{ route_path }}` "
                    "to the FastAPI router in {{ file_path }}. "
                    "Include a basic Pydantic model for request/response."
                ),
                parameters=[
                    TemplateParameter(name="file_path", type=ParameterType.FILE, required=True),
                    TemplateParameter(
