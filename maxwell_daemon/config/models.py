...
class AgentConfig(BaseModel):
    ...
    concurrency_by_kind: dict[str, int] = Field(
        default_factory=dict,
        description=(
            "Optional per-kind concurrency caps applied on top of the global "
            "worker pool. Keys typically use issue modes such as 'plan' "
            "or 'implement'."
        ),
    )
    max_queue_depth: int = Field(
        ...
        ge=1,
        description=(
            "Maximum number of tasks allowed in the queue before submissions "
    ...
    task_live_retention_seconds: int = Field(
        ...
        ge=0,
        description=(
            "Seconds to keep terminal tasks in memory before evicting them "
            "to the database."
        ),
    )
