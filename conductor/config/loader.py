...
from conductor.config.models import ConductorConfig
...
def default_config_path() -> Path:
    override = os.environ.get("CONDUCTOR_CONFIG")
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_CONFIG_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".config"
    return base / "conductor" / "conductor.yaml"
...
def save_config(config: ConductorConfig, path: Path | str | None = None) -> Path:
    p = Path(path).expanduser() if path else default_config_path()
