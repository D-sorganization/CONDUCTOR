...
people install only the extras they need (e.g., `pip install conductor-agents[ollama]`).
"""
...
from conductor.backends.base import BackendError, ILLMBackend
    ...
    def create(self, name: str, config: dict[str, Any]) -> ILLMBackend:
        if name not in self._factories:
            raise BackendError(
                ...
                f"Missing SDK? Try: pip install conductor-agents[{name}]"
            )
...
def _autoload() -> None:
    for name in _BUILTIN_BACKENDS:
        try:
            importlib.import_module(f"conductor.backends.{name}")
